...
interface ArduinoFirmwareDialogProps {
  isOpen: boolean;
  onClose: () => void;
  generatedCode?: string;
}

export function ArduinoFirmwareDialog({
  isOpen,
  onClose,
  generatedCode,
}: ArduinoFirmwareDialogProps) {
  const [copied, setCopied] = useState(false);

  const defaultCode = `// ULTRON AI - Web Serial PLC Firmware para Arduino Nano
...
`;
  ...
  const arduinoCode = generatedCode || defaultCode;
      ...
      <div className="bg-slate-900 border border-white/10 rounded-xl p-6 shadow-2xl max-w-3xl w-full flex flex-col max-h-[85vh]">
        <div className="flex justify-between items-center mb-4 shrink-0">
          <h2 className="text-white font-mono text-lg flex items-center gap-2 font-bold tracking-widest">
            <Download size={20} className="text-google-blue" /> FIRMWARE ARDUINO
            (C++)
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="text-slate-400 text-sm mb-4 space-y-2 shrink-0">
          <p>
            Para emparejar correctamente el Hardware (Arduino) con el Agent
            Ultron en la web, requieres programar la placa para que pueda
            interpretar los comandos enviados a través de nuestro Web Serial
            API.
          </p>
          <ul className="list-disc ml-5 text-slate-300">
            <li>
              Copia este código y pégalo en tu <strong>Arduino IDE</strong>.
            </li>
            <li>Conecta tu placa, compílalo y súbelo.</li>
            <li>
              En Ultron haz clic en "SERIAL" y selecciona el puerto de tu
              Arduino.
            </li>
          </ul>
        </div>
        ...
        <div className="relative flex-1 min-h-0 bg-black/50 rounded-lg border border-white/10 p-4 font-mono text-xs sm:text-sm text-google-green overflow-hidden flex flex-col shadow-inner">
          ...
          >
            {copied ? (
              <Check size={16} className="text-google-blue" />
            ) : (
              <Copy size={16} />
            )}
